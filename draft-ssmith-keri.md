...
Any identifier-system security-overlay binds together an identifier, asymmetric key-pairs, and controllers. By ***identifier*** we mean some string of characters. By ***key-pairs*** we mean a set of asymmetric (public, private) cryptographic key pairs used to create and verify non-repudiable digital signatures. By ***controllers*** we mean the set of entities whose members each control a private key from the given set of ***key-pairs***. When those bindings are strong then the overlay is relatively invulnerable to attack.  Likewise, when those bindings are weak then the overlay's security is relatively vulnerable to attack. The bindings for a given identifier form a *triad*  of the set of controllers, the set of key-pairs, and the identifier. Explicitly, the set of controllers is bound to the set of key-pairs, the set of key-pairs is bound to the identifier, and the identifier is bound to the set of controllers. This binding triad can be diagrammed as a triangle where each side is a binding and the vertices are the identifier, the set of controllers, and the set of key-pairs.

With KERI all the bindings of the triad are strong because they are cryptographically verifiable with a cryptographic strength or level of approximately 128 bits. See the Appendix on cryptographic strength for more detail on the meaning of cryptographic strength. The bound triad is created as follows\:
* Each controller of each key-pair creates a private key or seed of sufficient cryptographic strength. from a cryptographic strength pseudo-random number generator (CSPRNG) {{CSPRNG}}.
